...
def take(payload: Any, limit: int, fields: list[str]) -> Any:
    if isinstance(payload, list):
        return [select_fields(item, fields) for item in payload[:limit]]

    return select_fields(payload, fields)


def show_json(path: Path, limit: int, fields: list[str]) -> None:
    payload = json.loads(path.read_text(encoding="utf-8"))
    total = len(payload) if isinstance(payload, list) else 1
    print(json.dumps(take(payload, limit, fields), ensure_ascii=False, indent=2))
    print(f"\n{path.relative_to(PROJECT_ROOT).as_posix()}  —  toplam kayit: {total}")


def show_csv(path: Path, limit: int, fields: list[str]) -> None:
...
def show(path: Path, limit: int, fields: list[str]) -> None:
    if path.is_dir():
    ...
    if suffix == ".json":
        show_json(path, limit, fields)
    elif suffix == ".csv":
        show_csv(path, limit, fields)
...
def main() -> int:
    ...
    try:
        show(resolve_inside_project(arguments.path), arguments.limit, fields)
