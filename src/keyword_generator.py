...
def validate_keyword_output(raw_output: Any, expected_ids: set[str]) -> list[KeywordItem]:
    ...
    for index, item in enumerate(raw_output):
        ...
        # Reject informational-intent phrasing ("how", "what is", "review",
        # "blog"). This is the code-level enforcement of the "transactional
        # intent" requirement from the project's background (Broder's query
        # intent taxonomy) -- it runs on every client's output, fake or live.
...
class FakeKeywordClient:
    ...
    Despite the name, this is the client that actually produced the 489
    keywords used in every reported run (see
    data/processed/generated_keywords_multicategory.metadata.json:
    provider "fake", execution_mode "fake"). OpenRouterKeywordClient below
    is a real, working LLM-based alternative (--provider openrouter) but it
    was only smoke-tested on 3 products; it never generated the keyword
    corpus behind the results in this report.
    """
    ...
    def generate_keywords(self, products: list[Product]) -> list[KeywordItem]:
        raw_items = []
        for product in products:
            # " fiyat" ("price") is appended to every keyword to force
            # transactional (buy-intent) search behavior, the same intent
            # category validate_keyword_output checks for below.
            raw_items.append(
                {
...
class OpenRouterKeywordClient:
    ...
    This is the real LLM-based path (--provider openrouter), validated with
    a 3-product live smoke test. The 489 keywords behind the reported
    results were generated by FakeKeywordClient above, not this class.
    """
